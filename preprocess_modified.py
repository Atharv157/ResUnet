...
def crop_image_mask(image_dir, mask_dir, mask_path, X_points, Y_points, split_height=224, split_width=224):
    ...
    for i in Y_points:
        for j in X_points:
            ...
            # Skip if the cropped image or mask has incorrect shape (i.e., smaller than 224x224 or empty)
            if new_image.shape[0] != split_height or new_image.shape[1] != split_width:
                continue
            if new_mask.shape[0] != split_height or new_mask.shape[1] != split_width:
                continue
            ...
            if np.any(new_mask):
                ...
                unique_vals, counts = np.unique(new_mask, return_counts=True)

                # Create a dictionary of {value: count}
                val_counts = dict(zip(unique_vals, counts))

                # Get the count of black (0) and white (255) pixels safely
                num_black_pixels = val_counts.get(0, 0)
                num_white_pixels = val_counts.get(255, 0)

                # Now do the ratio check
                if num_black_pixels == 0 or num_white_pixels / num_black_pixels < 0.01:
                    num_skipped += 1
                    continue
...
if __name__ == '__main__':
    ...
    img_files = glob.glob(os.path.join(train_img_dir, '**', '*.jpg'), recursive=True)
    mask_files = glob.glob(os.path.join(train_mask_dir, '**', '*.jpg'), recursive=True)
