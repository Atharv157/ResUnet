...
def get_datasets(image_dir, mask_dir, val_size=0.2, seed=42):
    ...
    train_dataset = KvasirSegDataset(
        image_dir, mask_dir, image_list=train_imgs, transform=get_transforms(train=True)
    )
    val_dataset = KvasirSegDataset(
        image_dir, mask_dir, image_list=val_imgs, transform=get_transforms(train=False)
    )
